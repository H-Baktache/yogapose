...
// Main application when user is authenticated
const AppContent: React.FC = () => {
  const [loading, setLoading] = useState(true);
  const [activePage, setActivePage] = useState<'classifier' | 'sequence' | 'history'>('classifier');
  ...
  const handlePageChange = (page: 'classifier' | 'sequence' | 'history') => {
    setActivePage(page);
    window.location.hash = `#/${page === 'classifier' ? '' : page}`;
  };
    ...
    return (
      <div className="fixed inset-0 flex items-center justify-center bg-gradient-to-br from-indigo-50 to-white z-50">
        <div className="flex flex-col items-center">
          ...
          <p className="mt-4 text-indigo-700 font-medium bg-gradient-to-r from-indigo-600 to-purple-600 text-transparent bg-clip-text animate-pulse">Loading...</p>
        </div>
      </div>
    );
  }
      ...
      <div className="relative z-10 transition-all duration-1000 ease-out opacity-0 animate-fadeIn">
        ...
        <div className="container mx-auto px-4 py-4">
          ...
          <main className="flex-grow mx-auto mb-8">
            {activePage === 'classifier' && <YogaClassifier />}
            {activePage === 'sequence' && <YogaSequence />}
